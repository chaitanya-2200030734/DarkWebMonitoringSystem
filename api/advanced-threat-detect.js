...
async function fetchPageContent(url, timeout = 7000) {
  try {
    const isOnion = isOnionUrl(url);
    // Railway free tier has 10-second limit - optimize but keep functionality
    // Surface web: 7 seconds max, 1 retry
    // Onion: Not supported on Railway free tier (handled in fetch-url.js)
    const result = await fetchPageContentSecure(url, {
      timeout: timeout, // 7 seconds for surface web
      retries: 1, // 1 retry to stay within Railway's limit
...
export async function detectWebThreat(url) {
  ...
  // Google Safe Browsing (with timeout for Railway - but keep original functionality)
  try {
    const gsbPromise = checkGoogleSafeBrowsing(url);
    const gsbTimeout = new Promise((resolve) => setTimeout(() => resolve([false]), 3000));
    const [gsbFlagged] = await Promise.race([gsbPromise, gsbTimeout]);
    if (gsbFlagged) threats.malware = true;
  } catch (e) {
    // Skip if timeout or error - but don't break the flow
  }
  
  // PhishTank (with timeout for Railway - but keep original functionality)
  try {
    const ptPromise = checkPhishtank(url);
    const ptTimeout = new Promise((resolve) => setTimeout(() => resolve(false), 3000));
